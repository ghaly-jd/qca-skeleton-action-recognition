"""Kernel DTW baseline following Marteau-style recursive alignment kernels."""
...
class KDTWBaseline:
    """1-NN classifier using a normalized Kernel DTW similarity."""
    ...
    def fit(self, X_list: Sequence[np.ndarray], y: Sequence[Any]) -> "KDTWBaseline":
        ...
        if labels.size == 0:
            raise ValueError("Cannot fit KDTWBaseline on an empty dataset.")
    ...
    def predict(self, X_list: Sequence[np.ndarray]) -> np.ndarray:
        """Predict labels by nearest training sequence in KDTW kernel distance."""
    ...
    def similarity_matrix(self, X_list: Sequence[np.ndarray]) -> np.ndarray:
        """Return normalized KDTW similarities with shape ``N_test x N_train``."""
    ...
    def _check_is_fitted(self) -> None:
        if not hasattr(self, "train_sequences_"):
            raise RuntimeError("KDTWBaseline must be fitted before prediction.")
...
def kdtw_kernel(
    ...
    *,
    ...
    """Return a Kernel DTW similarity for two ``T x D`` sequences.

    The raw recursion sums soft alignment paths using an RBF local frame kernel.
    When ``normalize=True``, the return value is scaled by
    ``sqrt(K(x, x) K(y, y))`` so self-similarity is 1.0.
    """
    if gamma <= 0.0:
        raise ValueError("gamma must be positive.")

    x, y = _validate_pair(sequence_x, sequence_y)
    raw_kernel = _raw_kdtw_kernel(x, y, gamma=float(gamma))
    if not normalize:
        return raw_kernel

    self_x = _raw_kdtw_kernel(x, x, gamma=float(gamma))
    self_y = _raw_kdtw_kernel(y, y, gamma=float(gamma))
    return _normalize_kernel(raw_kernel, self_x, self_y)


def pairwise_kdtw_kernel(
    ...
    *,
    ...
    """Return pairwise KDTW similarities with shape ``N_test x N_train``."""
    ...
    if normalize:
        train_self = np.asarray(
            [
                kdtw_kernel(sequence, sequence, gamma=gamma, normalize=False)
...
def _raw_kdtw_kernel(
    sequence_x: np.ndarray,
    sequence_y: np.ndarray,
    *,
    gamma: float,
) -> float:
    local_kernel = _local_rbf_kernel_matrix(sequence_x, sequence_y, gamma=gamma)
    ...
    for i in range(1, n_frames_x + 1):
        for j in range(1, n_frames_y + 1):
            ...
            scores[i, j] = local_kernel[i - 1, j - 1] * predecessor_sum / 3.0
    return float(scores[n_frames_x, n_frames_y])


def _local_rbf_kernel_matrix(
    sequence_x: np.ndarray,
    sequence_y: np.ndarray,
    *,
    gamma: float,
    ...
    return np.exp(-float(gamma) * squared_distances).astype(np.float64, copy=False)
...
def _validate_pair(
    ...
    if x.ndim != 2 or y.ndim != 2:
        raise ValueError("KDTW inputs must be 2D T x D matrices.")
    if x.shape[0] == 0 or y.shape[0] == 0:
        raise ValueError("KDTW inputs must have at least one frame.")
    if x.shape[1] != y.shape[1]:
        raise ValueError("KDTW inputs must have the same feature dimension.")
    if not np.isfinite(x).all() or not np.isfinite(y).all():
        raise ValueError("KDTW inputs must contain only finite values.")
    return x, y
